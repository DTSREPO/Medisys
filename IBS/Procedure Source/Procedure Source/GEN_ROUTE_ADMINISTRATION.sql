...
create or replace procedure get_gen_route_proc (p_data_set varchar2, p_value OUT SYS_REFCURSOR) as
...
if array2.count>0 then
    for i in 1..array2.count loop
            FOR c in (select A.T_GEN_CODE GEN_CODE, B.T_LANG2_NAME GEN_DESC, A.T_FORMULA_CODE FRM_CODE, C.T_LANG2_NAME FORMULA_DESC, A.T_FORMULA_NOTES NOTES
                      ...
                      WHERE T_FORMULA_CODE='1022' AND A.T_FORMULA_CODE=C.T_FRMLRY_CODE AND A.T_GEN_CODE=B.T_GEN_CODE AND A.T_GEN_CODE=ARRAY2(I)
            ) loop
...
BEGIN
    get_gen_route_proc (
        ...
        p_value      =>route_cursor     -- generic set it will store in array then fetch by loop
    );
    LOOP
        FETCH route_cursor INTO v_gen_desc;

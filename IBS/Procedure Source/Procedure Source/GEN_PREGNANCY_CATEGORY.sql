...
if array2.count>0 then
    for i in 1..array2.count loop
            FOR c in (SELECT trim(ltrim(rtrim(A.T_GEN_CODE))) GEN_CODE, trim(ltrim(rtrim(A.T_LANG2_NAME))) GEN_DESC, trim(ltrim(rtrim(NVL(A.T_PREGNENCY_CAT,'N/A')))) CAT, trim(ltrim(rtrim(NVL(B.T_LANG2_NAME,'N/A')))) CAT_DESC
...
/* Result */
/*
GENTAMICIN|NO HUMAN STUDIES HAVE BEEN PERFORMED, ANIMAL STUDIES DEMONSTRATED A RISK TO THE FETUS. POTENTIAL BENEFIT MAY OUTWEIGH THE RISK.|1460C
LITHIUM CARBONATE|CONTROLLED STUDIES IN PREGNANT WOMEN SHOW NO A RISK TO THE FETUS.|1600A


PL/SQL procedure successfully completed.
*/
/* Wrong Paramerter Value */
/*
set serveroutput on;
declare
    preg_cat_cursor SYS_REFCURSOR;
...
BEGIN
    get_gen_preg_cat_proc (
        p_data_set   =>'1-2-1460160',      -- in parameter
        p_value      =>preg_cat_cursor     -- generic set it will store in array then fetch by loop
    );
    LOOP
        FETCH preg_cat_cursor INTO v_gen_desc;

/* Symptom_interection */
create or replace procedure get_sym_int_proc (p_data_set varchar2, p_type out varchar2, p_lang out varchar2, p_value OUT SYS_REFCURSOR) as
    ...
    /* Diseases interaction  */
        ...
        FOR I IN 1..ARRAY2.COUNT LOOP
            for j in (
                        ...
                        where a.T_ICD10_CODE=(B.T_ICD10_MAIN_CODE||''||B.T_ICD10_SUB_CODE) and A.T_SYMPTOM_FLAG='1' and A.T_GEN_CODE=array2(i)
            ) loop
...
set serveroutput on;
declare
    sym_cursor SYS_REFCURSOR;
    l_rec varchar2(30000);
BEGIN
    get_sym_int_proc (
        p_data_set  =>'1-2-1151160020861002',               -- in parameter
        p_value     =>sym_cursor                     -- generic set it will store in array then fetch by loop
    );

    LOOP
        FETCH sym_cursor INTO l_rec;
        EXIT WHEN sym_cursor%NOTFOUND;
...
/* Result */
/*
Car occupant injured in collision with heavy transport vehicle or bus, person on outside of vehicle, nontraffic accident, passenger van|Latest News and Information for the Rare Disease Community
Find A Registry #183; Find An Access Program
Categories: People, Treatment, Research, Finance|V4423
Car occupant injured in collision with heavy transport vehicle or bus, passenger, nontraffic accident, four-wheeled motor cycle|Latest News and Information for the Rare Disease Community
Find A Registry. Find An Access Program
Categories: People, Treatment, Research, Finance###;|V4412
...
BEGIN
    get_sym_int_proc (
        p_data_set  =>'1-2-14601',               -- in parameter
